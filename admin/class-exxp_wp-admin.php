...
class Exxp_wp_Admin {
    ...
    public function exxp_wp_custom_box_html($post)
    {
        ...
        if (get_post_status ($post_id) != 'publish')
        {
            ...
            $body .= "<script>function exxp_wp_createCORSRequest(){var e=\"" . exxp_wp_twoway_api_back . "/test_param\",t=new XMLHttpRequest;return\"withCredentials\"in t?t.open(\"POST\",e,!0):\"undefined\"!=typeof XDomainRequest?(t=new XDomainRequest).open(\"POST\",e):t=null,t}function exxp_wp_test_params(){document.getElementById(\"exxp_wp_status\").innerHTML=\"loading...\";var e=exxp_wp_createCORSRequest(),s=document.getElementsByName(\"steempress_sp_author\")[0].value,n=document.getElementsByName(\"steempress_sp_permlink\")[0].value,r=\"username=\"+s+\"&permlink=\"+n;e.setRequestHeader(\"Content-type\",\"application/x-www-form-urlencoded\"),e&&(e.username=s,e.permlink=n,e.onload=function(){var t=e.responseText;document.getElementById(\"exxp_wp_status\").innerHTML=\"ok\"===t?\"The parameters are correct. this article is linked to this <a target='_blank' rel='noopener noreferrer' href='https://hive.blog/@\"+this.username+\"/\"+this.permlink+\"'>hive post</a>\":\"Error : the permlink or username is incorrect.\"},e.send(r))}</script>";
