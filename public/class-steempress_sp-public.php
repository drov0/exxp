	...
	public function steempress_sp_comments($content)
    {
        ...
        if ($options['twoway'] == "on") {
            ...
            if (sizeof($username) == 1 and sizeof($permlink) == 1 and sizeof($tag) == 1) {
                ...
                if (is_front_page())
                    $steempress = "<iframe sandbox='allow-popups allow-scripts allow-same-origin' src=\"http://localhost:8002/?tag=".$tag."&author=".$username."&permlink=".$permlink."&display_comment=false\" style=\"border: 0; width: 100%;  max-height: 500px;\"></iframe>";
                else
                    $steempress = "<iframe sandbox='allow-popups allow-scripts allow-same-origin' src=\"http://localhost:8002/?tag=".$tag."&author=".$username."&permlink=".$permlink."&display_comment=true\" style=\"border: 0; width: 100%; max-height: 500px\"></iframe>";
